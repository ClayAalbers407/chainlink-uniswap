...
void ReadData() {
    ...
    if ((fp1 = fopen("D:\\lenovo_download\\in.txt", "rb")) == NULL) {
        printf("不能打开in.txt文件\n");
        exit(0);
    }
    //读取训练样本集，并存放在数组中
    for (int i = 0; i < Data; i++) {
        for (int j = 0; j < In; j++) {
            if (j != 0) {
                fscanf(fp1, "%c", &ch);//将，号存储到ch中，并指向下一个指针
            }
            fscanf(fp1, "%lf", &d_in[i][j]);
        }
    }
    fclose(fp1);
    if ((fp2 = fopen("D:\\lenovo_download\\out.txt", "rb")) == NULL) {
...
void TestNetwork() {
    ...
    char ch;
    //打开保存测试数据集的文件
    if ((fp = fopen("D:\\lenovo_download\\test.txt", "rb")) == NULL) {
